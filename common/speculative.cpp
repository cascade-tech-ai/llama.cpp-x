...
struct common_speculative_state_eagle3 : public common_speculative_state {
    ...
    int64_t pending_prompt_prefill_us = 0;
    double pending_prompt_prefill_gpu_ms = 0.0;
    ...
    void reset_prefix_cache() {
        ...
        pending_prompt_prefill_us = 0;
        pending_prompt_prefill_gpu_ms = 0.0;
    }
    ...
    size_t chunk_limit_from_env(const char * env_name, size_t default_chunk) const {
        ...
        } else if (const char * env = std::getenv("CASCADE_EAGLE_PREFILL_CHUNK")) {
            const long long parsed = std::strtoll(env, nullptr, 10);
            if (parsed > 0) {
                chunk = (size_t) parsed;
            }
        ...
        const size_t ubatch_cap = std::max<size_t>(1, llama_n_ubatch(ctx_tgt));
        return std::max<size_t>(1, std::min(chunk, ubatch_cap));
    }
    ...
    size_t generation_prefill_chunk_limit() const {
        return chunk_limit_from_env("CASCADE_EAGLE_GENERATION_PREFILL_CHUNK", 16);
    }
...
#if defined(GGML_USE_CUDA)
            if (profile_gpu) {
                LOG_INF("eagle3 profile prefill_call: phase=prompt prompt=%zu took=%.3fms gpu(took=%.3fms)\n",
                        prompt.size(),
                        pending_prompt_prefill_us / 1000.0,
                        pending_prompt_prefill_gpu_ms);
            } else
#endif
            {
                LOG_INF("eagle3 profile prefill_call: phase=prompt prompt=%zu took=%.3fms\n",
                        prompt.size(),
                        pending_prompt_prefill_us / 1000.0);
            }
        }
    ...
    void draft(
        ...
        int64_t t_prefill_us = pending_prompt_prefill_us;
        pending_prompt_prefill_us = 0;
        int64_t t_catchup_us = 0;
...
#if defined(GGML_USE_CUDA)
        double t_prefill_gpu_ms = pending_prompt_prefill_gpu_ms;
        pending_prompt_prefill_gpu_ms = 0.0;
        double t_catchup_gpu_ms = 0.0;
        ...
        const size_t prefix_prompt_len_before = prefix_prompt_len;
        const size_t delta_prompt = prompt_tgt.size() >= prefix_prompt_len_before
                ? prompt_tgt.size() - prefix_prompt_len_before
                : 0;
        ...
        // Materialize rollout-only KV if needed (prefix_state may have partial KV
        // from the fork path; the root step needs full KV for attention).
        if (root_state.rollout_only) {
            llama_eagle3_state full;
            if (llama_eagle3_rollout_materialize_state(*model, rt, root_state, full)) {
                root_state = std::move(full);
                // Update prefix_state too so catch-up step_multi gets full KV.
                prefix_state = root_state;
            }
        }

        const bool dump_root = dump_pending && !dump_dir.empty();
        llama_eagle3_step_debug dbg_root;
        if (delta_prompt > 0 && prefix_prompt_len_before > 0 && !dump_root) {
            if (!fetch_hidden_tensors(layer_tensors, n_tokens) || n_tokens < delta_prompt) {
                LOG_WRN("eagle3 draft: fetch_hidden_tensors failed or n_tokens=%zu < delta_prompt=%zu\n", n_tokens, delta_prompt);
                return;
            }
            if (prefix_tail_hidden_concat.size() != (size_t) hidden_in_dim) {
                LOG_WRN("eagle3 draft: prefix_tail_hidden_concat size %zu != hidden_in_dim %d\n", prefix_tail_hidden_concat.size(), hidden_in_dim);
                return;
            }

            const size_t generation_chunk = generation_prefill_chunk_limit();
            const size_t total_inputs = delta_prompt + 1;
            llama_eagle3_state chunk_base_state = prefix_state;
            bool have_next_prefix_state = false;

            for (size_t input_start = 0; input_start < total_inputs; input_start += generation_chunk) {
                const size_t input_end = std::min(total_inputs, input_start + generation_chunk);
                const bool includes_root = input_end == total_inputs;
                std::vector<llama_token> batched_input_ids;
                batched_input_ids.reserve(input_end - input_start);
                for (size_t i = input_start; i < input_end; ++i) {
                    batched_input_ids.push_back(
                            i < delta_prompt
                            ? prompt_tgt[prefix_prompt_len_before + i]
                            : id_last);
                }

                llama_eagle3_state next_state;
                llama_eagle3_state next_prefix_state;
                llama_eagle3_state * split_state_out = nullptr;
                int32_t split_after = 0;
                if (includes_root && input_start < delta_prompt) {
                    split_after = (int32_t) (delta_prompt - input_start);
                    split_state_out = &next_prefix_state;
                }

                const float * first_hidden_in = input_start == 0 ? prefix_tail_hidden_concat.data() : nullptr;
                const size_t token_idx_start = input_start == 0 ? 0 : (input_start - 1);
                const int64_t t_step_start = ggml_time_us();
                ++n_step_calls;
#if defined(GGML_USE_CUDA)
                ggml_backend_cuda_profiler_zone zone = {};
                if (profile_gpu) {
                    ggml_backend_cuda_profiler_zone_begin(
                            rt.backend_compute.get(),
                            &zone,
                            includes_root ? "eagle3/root_step" : "eagle3/catchup_prefill");
                }
#endif
                if (!llama_eagle3_step_multi_from_hidden_capture(
                            *model,
                            rt,
                            chunk_base_state,
                            first_hidden_in,
                            ...
                            split_after,
                            split_state_out,
                            includes_root ? root_state : next_state)) {
                    return;
                }
                const int64_t t_step_elapsed = ggml_time_us() - t_step_start;
                t_step_us += t_step_elapsed;
                if (includes_root) {
                    t_root_step_us += t_step_elapsed;
                } else {
                    t_catchup_us += t_step_elapsed;
                }
#if defined(GGML_USE_CUDA)
                if (profile_gpu) {
                    const double gpu_ms = ggml_backend_cuda_profiler_zone_end(
                            rt.backend_compute.get(),
                            &zone,
                            includes_root ? "eagle3/root_step" : "eagle3/catchup_prefill");
                    if (includes_root) {
                        t_root_step_gpu_ms += gpu_ms;
                    } else {
                        t_catchup_gpu_ms += gpu_ms;
                    }
                }
#endif
                if (includes_root) {
                    if (split_state_out) {
                        prefix_state = std::move(next_prefix_state);
                        have_next_prefix_state = true;
                    }
                } else {
                    chunk_base_state = std::move(next_state);
                }
            }

            if (!have_next_prefix_state) {
                prefix_state = std::move(chunk_base_state);
            }
            prefix_prompt_len = prompt_tgt.size();
            if (!build_hidden_concat_host(layer_tensors, delta_prompt - 1, prefix_tail_hidden_concat)) {
                return;
            }
            prefix_tail_capture_idx = delta_prompt - 1;
        } else {
            const int64_t t_prefill_start = ggml_time_us();
#if defined(GGML_USE_CUDA)
            ggml_backend_cuda_profiler_zone zone = {};
            if (profile_gpu) {
                ggml_backend_cuda_profiler_zone_begin(
                        rt.backend_compute.get(),
                        &zone,
                        prefix_prompt_len_before == 0 ? "eagle3/prompt_prefill" : "eagle3/catchup_prefill");
            }
#endif
            if (!prefill_to(prompt_tgt, seq_id)) {
                LOG_WRN("eagle3 draft: prefill_to failed\n");
                return;
            }
            const int64_t t_prefill_elapsed = ggml_time_us() - t_prefill_start;
            if (prefix_prompt_len_before == 0) {
                t_prefill_us += t_prefill_elapsed;
            } else {
                t_catchup_us += t_prefill_elapsed;
            }
#if defined(GGML_USE_CUDA)
            if (profile_gpu) {
                const double gpu_ms = ggml_backend_cuda_profiler_zone_end(
...
#if defined(GGML_USE_CUDA)
                if (profile_gpu) {
                    LOG_INF("eagle3 profile prefill_call: phase=%s prompt=%zu took=%.3fms gpu(took=%.3fms)\n",
                            prefix_prompt_len_before == 0 ? "prompt" : "catchup",
                            prompt_tgt.size(),
                            t_prefill_elapsed / 1000.0,
                            prefix_prompt_len_before == 0 ? t_prefill_gpu_ms : t_catchup_gpu_ms);
                } else
#endif
                {
                    LOG_INF("eagle3 profile prefill_call: phase=%s prompt=%zu took=%.3fms\n",
                            prefix_prompt_len_before == 0 ? "prompt" : "catchup",
                            prompt_tgt.size(),
                            t_prefill_elapsed / 1000.0);
                }
            ...
            if (!fetch_hidden_tensors(layer_tensors, n_tokens)) {
                LOG_WRN("eagle3 draft: fetch_hidden_tensors(2) failed, n_tokens=%zu\n", n_tokens);
                return;
            }
        ...
        if (!llama_eagle3_state_has_hidden(root_state)) {
            LOG_WRN("eagle3 draft: root_state has no hidden after step\n");
            return;
        }
        ...
        // Populate rollout batch prefix: copy root_state's KV to all beam slots.
        // This enables the O(depth) fork path in step_batch_from_parents.
        bool rollout_prefix_ok = false;
        if (rt.backend_compute && rt.buft_compute) {
            rollout_prefix_ok = llama_eagle3_rollout_populate_prefix(
                    *model, rt, root_state, beam_width, max_depth);
        }

        if (!rollout_prefix_ok && rt.backend_compute && rt.buft_compute) {
            // Fall back to old rollout batch setup.
        ...
        if (rollout_prefix_ok && llama_eagle3_fused_rollout_available(rt) && adaptive_depth_threshold <= 0.0f) {
            ...
            ggml_backend_cuda_profiler_zone zone = {};
            if (profile_gpu) {
                ggml_backend_cuda_profiler_zone_begin(rt.backend_compute.get(), &zone, "eagle3/fused_rollout");
            }
#endif
            const int64_t t_fused_start = ggml_time_us();
            llama_eagle3_fused_rollout_result fused_result;
            fused_ok = llama_eagle3_fused_rollout(
                    *model, rt, root_state, beam_width, max_depth, k, prob_threshold, fused_result);
#if defined(GGML_USE_CUDA)
            if (profile_gpu) {
                const double fused_gpu = ggml_backend_cuda_profiler_zone_end(
                        rt.backend_compute.get(), &zone, "eagle3/fused_rollout");
                t_logits_gpu_ms += fused_gpu;
                t_step_gpu_ms += fused_gpu;
            }
#endif
            if (fused_ok) {
                const int64_t t_fused_us = ggml_time_us() - t_fused_start;
                t_logits_us += t_fused_us / 2;
                t_step_us += t_fused_us / 2;
                n_logits_calls += max_depth;
                n_step_calls += max_depth;

                // Reconstruct all_nodes, prefix_states, proposal tree from journal.
                const int32_t ad = fused_result.actual_depth;
                const int32_t nb = fused_result.n_beams;
                // Build token paths from parent chain.
                // paths[depth * nb + beam] = token sequence from root to (depth, beam).
                std::vector<llama_tokens> paths((size_t) ad * nb);

                for (int32_t d = 0; d < ad; ++d) {
                    for (int32_t b = 0; b < nb; ++b) {
                        const size_t idx = (size_t) d * nb + b;
                        const int32_t parent_beam = fused_result.parents[idx];
                        const int32_t token = fused_result.tokens[idx];
                        const float logprob = fused_result.logprobs[idx];

                        // Skip invalid entries (inactive beams have garbage).
                        if (token < 0 || token >= model->hparams.vocab_size || logprob < -1e20f) {
                            continue;
                        }

                        llama_tokens path;
                        if (d > 0 && parent_beam >= 0 && parent_beam < nb) {
                            const size_t parent_idx = (size_t)(d - 1) * nb + parent_beam;
                            path = paths[parent_idx];
                        }
                        ...
                        all_nodes.push_back({logprob, path});
                        common_speculative_trace_insert_path(
                                last_trace.proposal_tree, path,
                                std::exp(logprob), std::exp(logprob));
                    }
                }

                // Fill in depth profiling stats for consistency.
                for (int32_t d = 0; d < ad; ++d) {
                    int32_t n_valid = 0;
                    for (int32_t b = 0; b < nb; ++b) {
                        const size_t idx = (size_t) d * nb + b;
                        if (fused_result.logprobs[idx] > -1e20f &&
                            fused_result.tokens[idx] >= 0 &&
                            fused_result.tokens[idx] < model->hparams.vocab_size) {
                            ++n_valid;
                        }
                    }
                    depth_active_beams.push_back(n_valid);
                    depth_expansions.push_back(n_valid);
                    depth_select_ms.push_back(0.0);
                    depth_score_ms.push_back(0.0);
                    depth_step_ms.push_back(0.0);
#if defined(GGML_USE_CUDA)
                    depth_select_gpu_ms.push_back(0.0);
                    depth_step_gpu_ms.push_back(0.0);
#endif
                }
            }
        }

        // ---- Unfused rollout path (fallback) ----
        if (!fused_ok) {
        // Set up beam 0 for unfused path (may need re-init if fused attempt failed).
        if (all_nodes.empty()) {
 ...
 #if defined(GGML_USE_CUDA)
            if (profile_gpu) {
                LOG_INF("eagle3 profile catchup: prompt=%zu catchup=%.3fms gpu(catchup=%.3fms)\n",
                        prompt_tgt.size(),
                        t_catchup_us / 1000.0,
                        t_catchup_gpu_ms);
            } else
 #endif
            {
                LOG_INF("eagle3 profile catchup: prompt=%zu catchup=%.3fms\n",
                        prompt_tgt.size(),
                        t_catchup_us / 1000.0);
            }
    ...
    void accept_tokens(const llama_tokens & ids, llama_seq_id seq_id) override {
        ...
        // Advance prefix_state to cover the accepted tokens.
        //
        // If any were accepted, rebuild K/V at those positions via the kv-only
        // fast path using teacher hiddens from the verification batch. This
        // replaces the rollout-chained K/V the speculative draft wrote into
        // those slots (which came from the draft head's own chained hidden
        // outputs) with K/V derived from the teacher's AR hiddens at those
        // same positions — matching the AR semantics the EAGLE head was
        // trained under. With no accepts, the root step's state (teacher-based
        // K/V at slot PL, plus 1 fallback token processed outside this path)
        // is the correct seed for the next cycle.
        if (accepted_nodes.empty()) {
            ...
            for (int32_t node : accepted_nodes) {
                ...
                if (r >= n_tokens) {
                    return; // malformed tree metadata; give up on this accept
                }
                accepted_rows_vec.push_back((int32_t) r);
            }

            // Tokens paired with those teacher hiddens: ids[1..N]. ids[0] was
            // the root-step input (already consumed); ids[N] is the fallback
            // (target-sampled, not in draft tree). EAGLE prefill convention:
            ...
            if (!llama_eagle3_ar_kv_regen(
                        *model, rt, last_root_state,
                        layer_tensors, accepted_rows_vec, slot_tokens,
                        regen_state)) {
                LOG_WRN("eagle3 accept: kv_regen failed, skipping state update\n");
                return;
            }

            // Deepest accepted row is the teacher-hidden source for the NEXT
            // cycle's root step (input to step_from_hidden_capture).
...
private:
    bool fetch_hidden_tensors(
        ...
        for (int32_t layer_id : layer_ids) {
            ...
            if (!data || n_layer_tokens == 0) {
                LOG_WRN("eagle3 fetch_hidden: layer %d has no data (data=%p, n_layer_tokens=%zu)\n", layer_id, (const void*)data, n_layer_tokens);
                return false;
            }
            if (tensors.empty()) {
    ...
    bool prefill_to(const llama_tokens & prompt_tgt, llama_seq_id seq_id) {
        GGML_UNUSED(seq_id);
        int64_t t_prefill_pass_us = 0;
        int32_t n_prefill_passes = 0;
#if defined(GGML_USE_CUDA)
        double t_prefill_pass_gpu_ms = 0.0;
#endif
        ...
        size_t n_tokens = 0;
        if (!fetch_hidden_tensors(layer_tensors, n_tokens)) {
            LOG_WRN("eagle3 prefill_to: fetch_hidden_tensors failed\n");
            return false;
        }
        if (prefix_prompt_len == 0 && n_tokens < prompt_tgt.size()) {
            LOG_WRN("eagle3 prefill_to: n_tokens=%zu < prompt_tgt.size()=%zu\n", n_tokens, prompt_tgt.size());
            return false;
        }
        ...
        const bool chunked_initial_prefill = !dump_steps && prefix_prompt_len == 0;
        const bool use_fixed_kv_prefill = chunked_initial_prefill &&
                std::getenv("CASCADE_EAGLE_OLD_PREFILL") == nullptr;
        if (use_fixed_kv_prefill) {
            const size_t prefill_chunk = prompt_prefill_chunk_limit();
            const size_t total_steps = prompt_tgt.size() - 1;

            const int64_t t_pass_start = ggml_time_us();
            n_prefill_passes = 1;
#if defined(GGML_USE_CUDA)
            ggml_backend_cuda_profiler_zone zone = {};
            if (profile_gpu) {
                ggml_backend_cuda_profiler_zone_begin(rt.backend_compute.get(), &zone, "eagle3/prefill_chunked");
            }
#endif
            if (!llama_eagle3_prefill_chunked(
                        *model,
                        rt,
                        hidden_in_dim,
                        layer_tensors,
                        ...
                        prefix_state)) {
                LOG_WRN("eagle3 prefill_to: prefill_chunked failed (total_steps=%zu, chunk=%zu, hidden_in_dim=%d, n_layers=%zu)\n",
                        total_steps, prefill_chunk, hidden_in_dim, layer_tensors.size());
                return false;
            }
#if defined(GGML_USE_CUDA)
            if (profile_gpu) {
                t_prefill_pass_gpu_ms += ggml_backend_cuda_profiler_zone_end(rt.backend_compute.get(), &zone, "eagle3/prefill_chunked");
            }
#endif
            t_prefill_pass_us += ggml_time_us() - t_pass_start;
        } else if (chunked_initial_prefill) {
            // Legacy path: per-chunk graph rebuild with full KV copy
            const size_t prefill_chunk = prompt_prefill_chunk_limit();
            const size_t total_steps = prompt_tgt.size() - 1;
            const int64_t t_old_loop_start = ggml_time_us();
            for (size_t chunk_start = 0; chunk_start < total_steps; chunk_start += prefill_chunk) {
                const size_t chunk_end = std::min(total_steps, chunk_start + prefill_chunk);
                std::vector<llama_token> input_ids;
                input_ids.reserve(chunk_end - chunk_start);
                for (size_t i = chunk_start + 1; i <= chunk_end; ++i) {
                    input_ids.push_back(prompt_tgt[i]);
                }
                ...
                ggml_backend_cuda_profiler_zone zone = {};
                if (profile_gpu) {
                    ggml_backend_cuda_profiler_zone_begin(rt.backend_compute.get(), &zone, "eagle3/prefill_pass");
                }
#endif
                if (!llama_eagle3_step_multi_from_hidden_capture(
                            *model,
...
#if defined(GGML_USE_CUDA)
                if (profile_gpu) {
                    t_prefill_pass_gpu_ms += ggml_backend_cuda_profiler_zone_end(rt.backend_compute.get(), &zone, "eagle3/prefill_pass");
                }
#endif
                t_prefill_pass_us += ggml_time_us() - t_pass_start;
                prefix_state = std::move(next_prefix_state);
            ...
            if (profile) {
                ...
                const double old_loop_ms = (ggml_time_us() - t_old_loop_start) / 1000.0;
                fprintf(stderr, "eagle3 prefill_old: tokens=%zu chunk=%zu passes=%d total=%.3fms avg=%.3fms\n",
                        total_steps, prefill_chunk, n_prefill_passes,
                        old_loop_ms, n_prefill_passes > 0 ? old_loop_ms / n_prefill_passes : 0.0);
            }
        } else if (prefix_prompt_len > 0) {
        ...
        const size_t start = chunked_initial_prefill ? prompt_tgt.size() : (prefix_prompt_len > 0 ? prefix_prompt_len : 0);
...
#if defined(GGML_USE_CUDA)
            if (profile_gpu) {
                LOG_INF("eagle3 profile prefill_passes: prompt=%zu passes=%d total=%.3fms avg=%.3fms gpu(total=%.3fms,avg=%.3fms)\n",
                        prompt_tgt.size(),
                        n_prefill_passes,
                        t_prefill_pass_us / 1000.0,
                        (t_prefill_pass_us / 1000.0) / n_prefill_passes,
                        t_prefill_pass_gpu_ms,
                        t_prefill_pass_gpu_ms / n_prefill_passes);
            } else
#endif
            {
                LOG_INF("eagle3 profile prefill_passes: prompt=%zu passes=%d total=%.3fms avg=%.3fms\n",
                        prompt_tgt.size(),
                        n_prefill_passes,
                        t_prefill_pass_us / 1000.0,
                        (t_prefill_pass_us / 1000.0) / n_prefill_passes);
